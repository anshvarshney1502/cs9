...
### Initial Wireframe
![Initial_wireframe](wireframe-1.png)
...
flowchart LR
    ...
    PublicUser --> BrowseFAQs(["Browse Published FAQs"])
    
    User --> RaiseQuery(["Raise a Query"])
    User --> AnswerComment(["Answer & Comment"])
    User --> VoteContent(["Vote on Content"])
    User --> ReportFlag(["Report/Flag Content"])
    User --> AcceptRes(["Accept Resolution"])
    
    Admin --> ViewMetrics(["View Admin Dashboard Metrics"])
    Admin --> ModerateFlags(["Moderate Flags"])
    Admin --> ManageUsers(["Manage Users & Roles"])
    Admin --> ExportFAQ(["Export Resolved Query to FAQ"])
    Admin --> ManageTags(["Manage Tags & Settings"])
```

### Dataflow Workflow (Question to FAQ)
...
sequenceDiagram
    ...
    participant Admin
    participant FAQ_Database

    User->>Platform: Submits new question
    Platform-->>User: Question becomes visible
    OtherUser->>Platform: Provides an answer
    User->>Platform: Accepts answer as resolution
    Admin->>Platform: Reviews resolved question
    Admin->>Platform: Clicks "Export to FAQ"
    Platform->>FAQ_Database: Clones Q&A into FAQ format
    FAQ_Database-->>Platform: FAQ Published
```
